...
def extract_wav2vec2_features(filepath, processor, model, device):
    try:
        ...
        features = {f"wav2vec2_{i}": val for i, val in enumerate(emb_mean)}

        return features

    except Exception as e:
        print(f"\nПомилка вилучення Wav2Vec2 для {filepath}: {e}")
        return None


def process_wav2vec2(
    metadata_path: str,
    gold_dir: str,
    use_ravdess: bool,
    use_iemocap: bool,
    ...
    for idx, row in tqdm(
        df_filtered.iterrows(), total=len(df_filtered), desc="Вилучення Wav2Vec2 ознак"
    ):
        ...
        feats = extract_wav2vec2_features(filepath, processor, model, device)
        if feats is None:
            feats = {}
    ...
    final_df = final_df.dropna(subset=["wav2vec2_0"])
    ...
    print(
        f"Успішно збережено {len(final_df)} рядків та {len(final_df.columns)} колонок. Шлях файлу {output_path}"
    )


def main():
    parser = argparse.ArgumentParser(description="Вилучення ознак Wav2Vec2.0")
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="wav2vec2_features.parquet",
